...
gulp.task("copy", function () {
  ...
  //jquery
  gulp.src("./node_modules/jquery/dist/jquery.min.js").pipe(gulp.dest("./build/js/"));
    ...
    return $.if(!isWindows, gulp.src(sketchPath + "*.sketch"), gulp.src(svgPath + "*.svg"))
      ...
      .pipe(
        $.iconfont({
          ...
          gulp
            .src("./src/.hidden/_font-template.scss")
          ...
          gulp
            .src("./src/.hidden/font-template.html")
            .pipe($.consolidate("underscore", options))
            .pipe(
              $.rename({
                basename: "iconfonts",
...
gulp.task("server", ["build"], function () {
  ...
  browser.init({
    ...
    injectChanges: true,
    files: [webPath + "{layouts,pages,partials}/**/*.html", sassPath + sassFiles, jsPath + jsFiles, jsPath + "**/*.js"],
    server: {
      baseDir: "./build",
    },
          ...
          inquirer
            ...
            .then(function (check) {
              if (check.build === answer1) {
                ...
                sequence("clean", "Iconfont", ["pages", "css", "js", "images", "copy"], "backup-build", done);
            ...
            .then(function (ans) {
              if (ans.yes) {
                sequence("clean", "Iconfont", ["pages", "css", "js", "images", "copy"], "deploy", function () {
                  console.log(
                    "---------------- Your Project has been successfully build and deployed to the server  ---------------- "
                  );
                });
              } else {
