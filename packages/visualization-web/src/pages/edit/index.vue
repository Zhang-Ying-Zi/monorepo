  ...
  <Header type="white" class="edit-menu">
    <template #pageTitle>
      <div class="page-title">
        ...
        <a-input class="title-content" :value="editState.pageConfig.config.projectName" @input="changeProjectName" />
      </div>
    </template>
    <template #menu>
      <a-menu-item @click="rollback"> <UndoOutlined />撤销 </a-menu-item>
      <a-menu-item @click="next"> <RedoOutlined />前进 </a-menu-item>
      <a-menu-item @click="() => saveConfig()"> <SaveOutlined />保存 </a-menu-item>
      <a-menu-item @click="() => setPreview(true, store)"> <EyeOutlined />预览 </a-menu-item>
...
export default {
  setup() {
    ...
    Promise.all([project.query({ id: router.query.id }), component.query()]).then(
    ...
    const initConfig = () => {
      ...
      watch([() => editState.pageConfig.userSelectComponents, () => editState.editConfig.currentIndex], () => {
