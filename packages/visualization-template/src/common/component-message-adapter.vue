...
<template>
  <div id="slider-view" class="slider-view" v-if="loaded">
    ...
    <div
      ...
      :id="`render-id-_component_${index}`"
      :key="index"
...
<script>
// window.__vslz_config__ = {
//   components: [
//     {
//       "name": "components-loader",
//       "config": {
//         "name": "global-banner.0.0.1",
...
import { postMsgToParent, isEdit, config, isPreview, baseUrl, pageId, xhrGet } from "./utils";

// document.domain = "visualization.cn"; // 设置同域

import RemoteComponentsLoader from "./remote-component-loader";

export default {
  name: "component-message-adapter",
  data() {
    return {
      init: false, // 内嵌 iframe 初始化完成
      ...
      isEdit,
      components: window.__vslz_config__.components.length
        ? window.__vslz_config__.components // window.__vslz_config__.components 是服务端注入的用户选择组件
        : this.$slots.default.map((c) => {
            console.log(c);
            const name = c.componentOptions?.tag || c.tag;
      ...
      page: {
        ...
        props: (window.__vslz_config__.pageData && window.__vslz_config__.pageData.props) || config.pageConfig.data,
      },
    };
  },
  created() {
