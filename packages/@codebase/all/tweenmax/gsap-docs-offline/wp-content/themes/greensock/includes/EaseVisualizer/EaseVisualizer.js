...
(function ($) {
  var htmlLink =
    "/wp-content/themes/greensock/includes/EaseVisualizer/EaseVisualizer.html";
  var id = 0;
  var timeline;
  var $menuEases;
  var highlightTween;
  ...
  function loaded(html, vis) {
    ...
    $(".ease_menu")
      ...
      .on("click", ".ease_class", { vis: vis }, onMenuEaseClick);
    ...
    vis
      .find(".go")
      .css("width", easeMenuWidth + "px")
      .on("click.easeVisualizer", { vis: vis }, onClickRun);

    // select options
    vis
      .find("select, input")
      .on("change.easeVisualizer", { vis: vis }, selectChange)
      .each(function () {
        var t = $(this);
        t.wrap("<label class='" + t.data("type") + "_label'></label>").after(
    ...
    vis
      .find(".editable")
      .attr("tabindex", "-1")
      .on("change.easeVisualizer", function (e) {
    ...
    if (data.settings.lockEase !== true) {
      vis
        ...
        .on("click", "button", { vis: vis }, easeChange)
        .trigger("change");
      vis
        .find(".main_ease_class_label")
        .on("mousedown.easeVisualizer", { vis: vis }, showVisSelect);
    } else {
    ...
    vis
      .find(".custom_path")
      .on("focusout.easeVisualizer", { vis: vis }, onInputCustomPath);
    ...
    if (i !== -1) {
      ...
      //in case "CustomEase" is selected initially but no actual ease data passed into the URL, we must first default to an ease to trace, so we use power2.out.
  ...
  function highlightMenuEase(name) {
    ...
    if (highlightTween) {
      //a simple, performant way to unhighlight the previous ease.
  ...
  function easeChange(e) {
    ...
    main_ease_class_select
      .find('option[value="' + val[0] + '"]')
      .prop("selected", true)
      .trigger("change");

    if (val[1] && val[1] !== "easeNone") {
      basic_ease_type_select
        .find('option[value="' + val[1] + '"]')
        .prop("selected", true)
        .trigger("change");
    }
  ...
  function selectChange(e) {
    ...
    customMode(
      vis,
      vis.data("easeVisualizer").currentEaseName === "CustomEase"
    );
    run(null, vis);
  }
    ...
    if (vis.custom !== enabled) {
      ...
      if (enabled && vis.find(".target_select").val() === "graph") {
        ...
        gsap.fromTo(
          ".ease-instructions",
          { y: 50 },
          { duration: 0.3, autoAlpha: 1, y: 0, delay: 0.2 }
        );
        path.setAttribute(
          "d",
          CustomEase.getSVGData(ease, { width: 500, height: 500 })
        );
        if (vis.editor) {
          ...
          vis.editor = new PathEditor(path, {
            ...
            onUpdate: function () {
              var hadError = hasError,
                ...
                firstX = rawPath[0][0];
              hasError = false;
              text.innerHTML = this.getNormalizedSVG(
                500,
                500,
                true,
                onEaseError
  ...
  function onInputCustomPath(e) {
    ...
    if (vis && vis.custom) {
      ...
      ease = CustomEase.create("custom", data, {
      ...
      vis
        .find(".ease_template")
        .attr("d", vis.find(".graph_path_reveal").attr("d"));
      vis.editor.init();
    }
  }
  ...
  function checkVertical(vis) {
    ...
    var tall =
      ...
      (main_ease_class_select === "CustomEase" ||
        ...
        (main_ease_class_select === "RoughEase" &&
    ...
    if (tall) {
      ...
      gsap.to(vis.find(".ease_menu"), {
        duration: 0.4,
        top: 220,
        ease: "power2.inOut",
      });
      gsap.to(vis.find("#graph_path").find("rect"), {
        duration: 0.5,
        attr: { y: -200 },
        ease: "power2.inOut",
      });
    } else {
      //we decided not to animate BACK after going tall because it's annoying.
  ...
  function refreshTween(vis) {
    ...
    var ease,
      c = vis.find(".main_ease_class_select").val(),
    ...
    switch (c) {
      ...
      case "RoughEase":
        var strength = parseFloat(
          vis.find(".rough_strength").siblings(".display").text(),
          10
        );
        ...
        var points = parseFloat(
          vis.find(".rough_points").siblings(".display").text(),
          10
        );
        ...
        ease =
          ...
          template +
          ",taper:" +
          vis.find(".rough_taper_select").val().replace(/\"/g, "") +
          ",randomize:" +
          vis.find(".rough_randomize_checkbox").prop("checked") +
          ",clamp:" +
          vis.find(".rough_clamp_checkbox").prop("checked") +
          "})";
        break;

      case "SlowMo":
        var ratio = parseFloat(
          vis.find(".slowmo_ratio").siblings(".display").text(),
          10
        );
        ...
        var power = parseFloat(
          vis.find(".slowmo_power").siblings(".display").text(),
          10
        );
      ...
      case "SteppedEase":
        var steps = parseInt(
          vis.find(".stepped_steps").siblings(".display").text(),
          10
        );
      ...
      case "Elastic":
        var amplitude =
            parseFloat(
              vis.find(".elastic_amplitude").siblings(".display").text()
            ) || 1,
          period = parseFloat(
            vis.find(".elastic_period").siblings(".display").text()
          );
      ...
      case "Back":
        var amount =
          parseFloat(vis.find(".back_amount").siblings(".display").text()) || 1;
      ...
      case "CustomEase":
        ease = CustomEase.create("custom", vis.find(".custom_path").text(), {
          height: 1,
        });
        break;
    ...
    if (
      !(c === "CustomEase" && previousEaseName !== "CustomEase") &&
      isLoaded
    ) {
      createGraph(vis);
    }
  ...
  function runGraphVis(vis) {
    ...
    var offset = 0.2;
    var graphTL = (timeline = gsap.timeline({
      delay: 0.1,
      defaults: { duration: d },
    ...
    graphTL.fromTo(
      vis.find(".graph_line"),
      { autoAlpha: 0 },
      { duration: 0.0001, autoAlpha: 1 },
      "start"
    );

    graphTL.fromTo(
      vis.find("#graph_path_reveal rect"),
      { attr: { width: 0 } },
      { attr: { width: 500 }, ease: "none" },
      "start"
    );
    graphTL.fromTo(
      vis.find(".graph_liney"),
      { attr: { x1: 0, x2: 0 } },
      { attr: { x1: 500, x2: 500 }, ease: "none" },
      "start"
    );
    graphTL.fromTo(
      vis.find(".progress_joint"),
      { top: "100%" },
      { top: "0%", ease: ease },
      "start"
    );
    graphTL.fromTo(
      vis.find(".graph_linex"),
      { attr: { y1: 500, y2: 500 } },
      { attr: { y1: 0, y2: 0 }, ease: ease },
      "start"
    );

    graphTL.fromTo(
      vis.find(".horizontal .progress_fill"),
      { scaleX: 0, transformOrigin: "left center" },
      { ease: "none", scaleX: 1 },
      "start"
    );
    graphTL.fromTo(
      vis.find(".vertical .progress_fill"),
  ...
  function runClockVis(vis) {
    ...
    clockTL.fromTo(
      ...
      { ease: ease, rotation: 360, force3D: true },
      "start"
    );
    clockTL.fromTo(
      vis.find(".clock_linear"),
      d,
      { rotation: 0, transformOrigin: "center bottom" },
      { ease: "none", rotation: 360, force3D: true },
      "start"
    );
  ...
  function runBoxVis(vis) {
    ...
    boxTL.fromTo(
      ...
      d,
      { x: "0%" },
      { ease: "power0.in", x: "400%", force3D: true },
      "start"
    );
    boxTL.fromTo(
      vis.find(".box_power1"),
      d,
      { x: "0%" },
      { ease: "power1." + easeType || power1.out, x: "400%", force3D: true },
      "start"
    );
    boxTL.fromTo(
      vis.find(".box_power2"),
      d,
      { x: "0%" },
      { ease: "power2." + easeType || power2.out, x: "400%", force3D: true },
      "start"
    );
    boxTL.fromTo(
      vis.find(".box_power3"),
      d,
      { x: "0%" },
      { ease: "power3." + easeType || power3.out, x: "400%", force3D: true },
      "start"
    );
    boxTL.fromTo(
      vis.find(".box_power4"),
      d,
      { x: "0%" },
      { ease: "power4." + easeType || power4.out, x: "400%", force3D: true },
      "start"
    );
    boxTL.fromTo(
      vis.find(".box_custom"),
  ...
  function createGraph(vis) {
    var name = vis.find(".main_ease_class_select").val(),
      ...
      precision =
        name === "Stepped" || name === "Bounce" || name === "Elastic" ? 3 : 1,
      ease = data.currentEase || "none",
      mainEase = data.currentEaseName,
      fullEaseName = mainEase + "." + data.currentEaseType,
      customString =
        (fullEaseName !== "elastic.out" && mainEase !== "back") ||
        (fullEaseName === "elastic.out" &&
          ease._p1 === 1 &&
          ease._p3 === 0.075) ||
        (mainEase === "back" && ease._p1 === 1.7)
          ? customStrings[fullEaseName]
          : null, //note: we created a simplified version of elastic.out and the back eases, but only with the default configuration so if there is any customization, we should just do the auto-tracing.
    ...
    if (customString) {
      ...
      path = CustomEase.getSVGData(ease, {
        width: 500,
        height: 500,
        precision: precision,
      });
      simplified =
        ease.rawBezier || ease.custom
          ? path
          : PathEditor.simplifySVG(path, {
              tolerance: precision === 1 ? 3 : 1,
              cornerThreshold:
                mainEase === "Bounce"
                  ? 130
                  : mainEase === "Stepped" || mainEase === "Rough"
                  ? 180
                  : 0,
            });
      $customPath.text(
...
function showBasicOverlay(overlay, onComplete) {
  ...
  if (activeOverlay) {
    //if there's already one open, immediately close it.
...
jQuery(".ease-visualizer").each(function () {
  ...
  $this
    ...
    .easeVisualizer({
      startEase: $this.data("ease") || "power2.out",
      lightTheme: $this.data("light"),
    });
});
