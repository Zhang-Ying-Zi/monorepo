...
_gsScope._gsDefine(
  ...
  function () {
    var _tempVarsXY = { css: {}, data: "_draggable" }, //speed optimization - we reuse the same vars object for x/y TweenLite.set() calls to minimize garbage collection tasks and improve performance.
      ...
      _isAndroid =
        _gsScope.navigator &&
        _gsScope.navigator.userAgent.toLowerCase().indexOf("android") !== -1, //Android handles touch events in an odd way and it's virtually impossible to "feature test" so we resort to UA sniffing
      _lastDragTime = 0,
      ...
      _getComputedStyle = function (e, s) {
        return _computedStyleScope.getComputedStyle(
      ...
      _isIE10orBelow =
        _gsScope.navigator &&
        (/MSIE ([0-9]{1,}[\.0-9]{0,})/.exec(_gsScope.navigator.userAgent) ||
      ...
      _getSVGOffsets = function (e) {
        ...
        if (
          ...
          e._dCache &&
          e._dCache.lastUpdate === TweenLite.ticker.frame
        ) {
      ...
      _getOffset2DMatrix = function (
        ...
        var cache = e._dCache || _cache(e),
          ...
          borderTranslateY;
        m =
          cache.isSVG && (e.style[_transformProp] + "").indexOf("matrix") !== -1
        ...
        if (offsetOrigin) {
          ...
          if (cache.isSVG) {
            ...
            if (
              parentOffsetParent === _doc.body &&
              offsets.offsetParent === _docElement
            ) {
          ...
          if (parent && _getStyle(e, "position", true) === "fixed") {
            ...
            !parentCache.isSVG &&
            (!_hasReparentBug ||
              _getOffset2DMatrix(parent).join("") === "100100")
          ) {
            ...
            if (
              ...
              m[4] -=
                parseInt(
                  _getStyle(parentCache.offsetParent, "borderLeftWidth"),
                  10
                ) || 0;
              m[5] -=
                parseInt(
                  _getStyle(parentCache.offsetParent, "borderTopWidth"),
                  10
                ) || 0;
            }
          }
      ...
      _localizePoint = function (p, localToGlobal, globalToLocal) {
        var x =
            p.x * localToGlobal[0] + p.y * localToGlobal[2] + localToGlobal[4],
          y =
            p.x * localToGlobal[1] + p.y * localToGlobal[3] + localToGlobal[5];
        ...
        if (e === window) {
          ...
          if (e.nodeName.toLowerCase() === "svg" && !_isOldIE) {
            ...
            right =
              left +
              width -
              (width - (width - borderLeft) / cache.scaleX - viewBoxX);
            bottom =
              top +
              height -
              (height - (height - borderTop) / cache.scaleY - viewBoxY);
      ...
      Draggable = function (target, vars) {
        ...
        var type = (vars.type || (_isOldIE ? "top,left" : "x,y")).toLowerCase(),
          ...
          animate = function (throwProps, forceZeroVelocity) {
            ...
            if (throwProps && ThrowPropsPlugin) {
                ...
                if (rotationMode) {
                  ...
                  if (allowX) {
                    throwProps[xProp] = _parseThrowProps(
                      ...
                      snapIsRaw
                        ? snap
                        : snap.points || snap.x || snap.left || snap.scrollLeft,
                      maxX,
                      minX,
                      scrollProxy ? -1 : 1,
                  ...
                  if (allowY) {
                    throwProps[yProp] = _parseThrowProps(
                      ...
                      snapIsRaw
                        ? snap
                        : snap.points || snap.y || snap.top || snap.scrollTop,
                      maxY,
                      minY,
                      scrollProxy ? -1 : 1,
                  ...
                  if (
                    snap.points ||
                    (snap instanceof Array && typeof snap[0] === "object")
                  ) {
              ...
              self.tween = tween = ThrowPropsPlugin.to(
                ...
                Math.max(vars.minDuration || 0, vars.maxDuration || 0) || 2,
                !isNaN(vars.minDuration)
                  ? vars.minDuration
                  : overshootTolerance === 0 ||
                    (typeof throwProps === "object" &&
                      throwProps.resistance > 1000)
                  ? 0
                  : 0.5,
                overshootTolerance
              );
          ...
          recordStartPositions = function () {
            ...
            if (matrix) {
              startPointerX =
                self.pointerX * matrix[0] +
                self.pointerY * matrix[2] +
                matrix[4]; //translate to local coordinate system
              startPointerY =
                self.pointerX * matrix[1] +
                self.pointerY * matrix[3] +
                matrix[5];
            }
            if (dirty) {
            ...
            if (scrollProxy) {
              ...
              if (rotationMode) {
                ...
                startElementY = self.y =
                  Math.atan2(
                    rotationOrigin.y - self.pointerY,
                    self.pointerX - rotationOrigin.x
                  ) * _RAD2DEG;
              } else {
            ...
            if (!rotationMode && (vars.cursor !== false || vars.activeCursor)) {
              ...
              while (--i > -1) {
                _setStyle(
                  triggers[i],
                  "cursor",
                  vars.activeCursor || vars.cursor || "move"
                );
              }
            }
            _dispatchEvent(self, "press", "onPress");
          ...
          onRelease = function (e, force) {
            ...
            var originalEvent = e,
              ...
              isContextMenuRelease =
                self.vars.allowContextMenu && e && (e.ctrlKey || e.which > 2),
              placeholderDelayedCall = TweenLite.delayedCall(
                0.001,
                removePlaceholder
              ),
              touches,
            ...
            if (!rotationMode) {
              ...
              while (--i > -1) {
                _setStyle(
                  triggers[i],
                  "cursor",
                  vars.cursor || (vars.cursor !== false ? "move" : null)
                );
              }
            }
            if (wasDragging) {
          ...
          if (type !== "soft") {
            ...
            while (--i > -1) {
              ...
              _setStyle(
                trigger,
                "touchAction",
                allowX === allowY ? "none" : allowX ? "pan-y" : "pan-x"
              );
              if (_isSVG(trigger)) {
                // a bug in chrome doesn't respect touch-action on SVG elements - it only works if we set it on the parent SVG.
                _setStyle(
                  trigger.ownerSVGElement || trigger,
                  "touchAction",
                  allowX === allowY ? "none" : allowX ? "pan-y" : "pan-x"
                );
              }
