      ...
      init: function (target, value, tween) {
        ...
        for (p in value) {
          ...
          if (p === "fill" || p === "stroke") {
            ...
            if ((s || s === 0) && (v || v === 0) && (pt.c = rel ? v : v - s)) {
              //faster than isNaN(). Also, we set pt.c (change) here because if it's 0, we'll just treat it like a non-tweening value. can't do (v !== start) because if it's a relative value and the CHANGE is identical to the START, the condition will fail unnecessarily.
              pt.s = s;
            } else {
              pt.type = -1;
  ...
  p._parseTransform = function (t, v) {
    ...
    if (typeof v === "object") {
      ...
      if (v.shortRotation != null) {
        m2.rotation =
          typeof v.shortRotation === "number"
            ? v.shortRotation * _DEG2RAD
            : _parseAngle(v.shortRotation, m1.rotation);
        ...
        m2.rotation =
          v.rotation == null
            ...
            ? v.rotation * _DEG2RAD
            : _parseAngle(v.rotation, m1.rotation);
      }
      m2.skewX =
        v.skewX == null
          ? m1.skewX
          : typeof v.skewX === "number"
          ? v.skewX * _DEG2RAD
          : _parseAngle(v.skewX, m1.skewX);

      //note: for performance reasons, we combine all skewing into the skewX and rotation values, ignoring skewY but we must still record it so that we can discern how much of the overall skew is attributed to skewX vs. skewY. Otherwise, if the skewY would always act relative (tween skewY to 10deg, for example, multiple times and if we always combine things into skewX, we can't remember that skewY was 10 from last time). Remember, a skewY of 10 degrees looks the same as a rotation of 10 degrees plus a skewX of -10 degrees.
      m2.skewY =
        v.skewY == null
          ? m1.skewY
          : typeof v.skewY === "number"
          ? v.skewY * _DEG2RAD
          : _parseAngle(v.skewY, m1.skewY);
