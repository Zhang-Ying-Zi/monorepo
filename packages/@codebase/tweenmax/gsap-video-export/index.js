...
console.log(
  `gsap-video-export ${version} / ${colors.blue}@defaced${colors.reset}`
);
...
const resolutions = {
  viewportWidth: parseInt(
    options.viewport.split("x")[0]
  ) /* An additional 16px is required because puppeteer is coming up short */,
...
const padCenter = (first, last, fail = false) => {
  ...
  return `${first}${padChar.repeat(pad)}${
    fail
      ? `${colors.red}${last}${colors.reset}`
      : `${colors.blue}${last}${colors.reset}`
  }`;
};
...
const exportVideo = async () => {
  ...
  await page.setViewport({
    width: resolutions.viewportWidth,
    height: resolutions.viewportHeight,
  ...
  console.log(
    padCenter(
      "Selector",
      validSelector ? `(${options.selector}) OK` : `(${options.selector}) FAIL`,
      !validSelector
    )
  );
  ...
  /* Print status text */
  console.log(
    padCenter("GSAP", gsapVersion ? "v" + gsapVersion : "FAIL", !gsapVersion)
  );
  ...
  console.log(
    padCenter(
      "Timeline",
      timeline ? `(${options.timeline}) OK` : "FAIL",
      !timeline
    )
  );
  ...
  console.log(
    padCenter(
      "Duration",
      durationSeconds !== 0 ? timeString(durationSeconds.toFixed(1)) : "FAIL",
      durationSeconds === 0
    )
  );
  ...
  for (let x = startFrame; x < endFrame; x++) {
    ...
    const el =
      options.selector === "document" ? page : await page.$(options.selector);
  ...
  console.log(
    padCenter(
      "Output resolution",
      `${options.resolution === "auto" ? "(auto) " : ""}${finalResolution}`
    )
  );
  console.log(
    padCenter(
      "Padding color",
      `${options.color === "auto" ? "(auto) " : ""}#${padColor.toUpperCase()}`
    )
  );
  ...
  const render = ffmpeg()
    ...
    .on("end", function () {
      ...
      console.log(
        padCenter(
          "Export",
          ((timeFramesStop[0] * 1e9 + timeFramesStop[1]) / 1e9)
            .toFixed(2)
            .toString() + "s",
          false
        )
      ...
      console.log(
        padCenter(
          "Render",
          ((timeRenderStop[0] * 1e9 + timeRenderStop[1]) / 1e9)
            .toFixed(2)
            .toString() + "s",
          false
        )
  ...
  if (options["input-options"])
    render.inputOptions(
      ...parseArgsStringToArgv(options["input-options"].slice(1, -1))
    );
  if (options["output-options"])
    render.outputOptions(
      ...parseArgsStringToArgv(options["output-options"].slice(1, -1))
    );
