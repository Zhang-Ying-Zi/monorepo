...
(function ($) {
  ...
  $.fn.animate = function (prop, speed, easing, callback) {
    ...
    if (
      !_enabled ||
      prop.skipGSAP === true ||
      (typeof speed === "object" && typeof speed.step === "function")
    ) {
      //we don't support the "step" feature because it's too costly performance-wise, so fall back to the native animate() call if we sense one. Same with scrollTop and scrollLeft which are handled in a special way in jQuery.
      return _animate.call(this, prop, speed, easing, callback);
    }
    var config = $.speed(speed, easing, callback),
      vars = {
        ease:
          ...
          (config.easing === false ? _easeMap.linear : _easeMap.swing),
      },
      obj = this,
    ...
    for (p in prop) {
      ...
      if (
        ...
        val === "hide" ||
        val === "toggle" ||
        (_legacyProps.indexOf(p) !== -1 &&
          _legacyProps.indexOf("," + p + ",") !== -1)
      ) {
        //note: slideUp() and slideDown() pass in opacity:"show" or opacity:"hide"
        return _animate.call(this, prop, speed, easing, callback);
      } else {
