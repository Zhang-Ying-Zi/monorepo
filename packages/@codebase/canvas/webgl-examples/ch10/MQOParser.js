...
MQODoc.prototype.readObjects = function () {
  ...
  while ((line = this.lines[this.index++])) {
    ...
    if (line.indexOf("shading") >= 0) {
      // Read shading parameter
...
MQODoc.prototype.updateNumVertices = function (numIndices) {
  ...
  if (numIndices == 3) this.numIndices += numIndices;
  // Add 3 (Draw a triangle)
  // numIndices == 4 (Rectangle)
  else this.numIndices += numIndices * 2; // Add 6 (Draw 2 triangles)
};

// Skip to the end of chunk
MQODoc.prototype.skipToEndOfChunk = function () {
  var line;
  while ((line = this.lines[this.index++])) if (line.indexOf("}") >= 0) break;
};
  ...
  for (var i = 0, nobj = this.objects.length; i < nobj; i++) {
    ...
    for (var j = 0, nface = obj.faces.length; j < nface; j++) {
      ...
      for (var k = 0, n = face.vIndices.length; k < n; k++) {
        ...
        var normal;
        if (obj.shading == 0)
          // Use either surface normal or vertex normal
          normal = face.normal;
        // Use surface normal
        else normal = v.normal; // Use vertex normal
      ...
      for (
        var l = 0;
        l < 3;
        l++ // Set indices of the vertices
      )
        indices[index_indices + l] = index_vertices / 3 + l;

      if (n == 4) {
        // In case of a rectangle, append another triangle
...
Face.prototype.setNormal = function (vertices) {
  ...
  // Check the normal
  if (this.normal == null) {
    if (this.vIndices.length == 4) {
      // If face is rectangle, calculate the normal with another 3 points
      var v3 = vertices[this.vIndices[3]];
      this.normal = calcNormal(v1.xyz, v2.xyz, v3.xyz);
    }
    if (this.normal == null) {
      // If failed to calculate the normal, set (0, 1, 0) to normal
