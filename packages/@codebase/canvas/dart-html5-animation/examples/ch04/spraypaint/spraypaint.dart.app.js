...
function $sar(x, y) {
  return typeof x == "number" && typeof y == "number" ? x >> y : x.$sar(y);
}
...
CanvasPixelArrayWrappingImplementation.prototype.removeLast = function () {
  $throw(
    new UnsupportedOperationException("Cannot removeLast on immutable List.")
  );
};
...
spraypaint.prototype.run = function () {
  ...
  function onMouseMove(e) {
    for (var i = 0; $lt(i, brush_density); i = $add(i, 1)) {
      var angle = Math.random() * 3.141592653589793 * 2,
        radius = $mul(Math.random(), brush_size),
        xpos = $bit_or($add(mouse.get$x(), $mul(Math.cos(angle), radius)), 0),
        ypos = $bit_or($add(mouse.get$y(), $mul(Math.sin(angle), radius)), 0),
        offset = $mul($add(xpos, $mul(ypos, imagedata.get$width())), 4);
