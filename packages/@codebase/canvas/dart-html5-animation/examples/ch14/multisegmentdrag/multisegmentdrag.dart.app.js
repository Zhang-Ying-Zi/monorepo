...
multisegmentdrag.prototype.run = function () {
  ...
  function drag(segment, xpos, ypos) {
    var dx = $sub(xpos, segment.get$x()),
      dy = $sub(ypos, segment.get$y());
    segment.set$rotation(Math.atan2(dy, dx));
    var w = $sub(segment.getPin().get$x(), segment.get$x()),
      h = $sub(segment.getPin().get$y(), segment.get$y());
  ...
  function move(segment, i) {
    if (null == i ? null != 0 : i !== 0) {
      drag(
        segment,
        segments.$index($sub(i, 1)).get$x(),
        segments.$index($sub(i, 1)).get$y()
      );
    }
  }
