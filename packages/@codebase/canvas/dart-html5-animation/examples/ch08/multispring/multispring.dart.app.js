...
multispring.prototype.run = function () {
  ...
  var numHandles = 3,
    ...
    friction = 0.9,
    movingHandle = null;
  for (var i = 0; $lt(i, numHandles); i = $add(i, 1)) {
  ...
  canvas
    ...
    .add(function (event) {
      handles.forEach$1(function (handle) {
        if (
          Utils.containsPoint(handle.getBounds(), mouse.get$x(), mouse.get$y())
        ) {
          movingHandle = handle;
        }
      });
  ...
  function applyHandle(handle) {
    var dx = $sub(handle.get$x(), ball.get$x()),
      dy = $sub(handle.get$y(), ball.get$y());
