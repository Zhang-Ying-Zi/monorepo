...
samemass.prototype.run = function () {
  ...
  function rotate(x, y, sin, cos, reverse) {
    return new point(
      reverse ? $mul(x, cos) + $mul(y, sin) : $mul(x, cos) - $mul(y, sin),
      reverse ? $mul(y, cos) - $mul(x, sin) : $mul(y, cos) + $mul(x, sin)
    );
  }
  function checkCollision(ball0, ball1) {
    var dx = $sub(ball1.get$x(), ball0.get$x()),
      dy = $sub(ball1.get$y(), ball0.get$y()),
    ...
    if ($lt(dist, ball0.get$radius() + ball1.get$radius())) {
      var angle = Math.atan2(dy, dx),
        ...
        vel0 = rotate(ball0.get$vx(), ball0.get$vy(), sin, cos, true),
        vel1 = rotate(ball1.get$vx(), ball1.get$vy(), sin, cos, true);
      var temp = vel0;
      vel0 = vel1;
      vel1 = temp;
      var absV = vel0.get$x().abs() + vel1.get$x().abs(),
        overlap =
          ball0.get$radius() +
          ball1.get$radius() -
          $sub(pos0.get$x(), pos1.get$x()).abs();
      pos0.set$x($add(pos0.get$x(), $mul($div(vel0.get$x(), absV), overlap)));
      pos1.set$x($add(pos1.get$x(), $mul($div(vel1.get$x(), absV), overlap)));
      var pos0F = rotate(pos0.get$x(), pos0.get$y(), sin, cos, false),
        pos1F = rotate(pos1.get$x(), pos1.get$y(), sin, cos, false);
      ...
      var vel0F = rotate(vel0.get$x(), vel0.get$y(), sin, cos, false),
        vel1F = rotate(vel1.get$x(), vel1.get$y(), sin, cos, false);
  ...
  function drawFrame(t) {
    ...
    for (
      var ballA, i = 0, len = $sub(numBalls, 1);
      $lt(i, len);
      i = $add(i, 1)
    ) {
      ballA = balls.$index(i);
      for (var ballB = null, j = $add(i, 1); $lt(j, numBalls); j = $add(j, 1)) {
        ballB = balls.$index(j);
...
Ball.prototype.get$mass = function () {
  return this.mass;
};
Ball.prototype.set$mass = function (value) {
