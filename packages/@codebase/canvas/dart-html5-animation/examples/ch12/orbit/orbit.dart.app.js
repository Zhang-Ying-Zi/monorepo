...
orbit.prototype.run = function () {
  ...
  var numParticles = 2;
  var sun = new Ball.withRadiusAndColor$ctor(100, "#ffff00"),
    planet = new Ball.withRadiusAndColor$ctor(10, "#00ff00");
  ...
  function gravitate(partA, partB) {
    var dx = $sub(partB.get$x(), partA.get$x()),
      dy = $sub(partB.get$y(), partA.get$y()),
      distSQ = $mul(dx, dx) + $mul(dy, dy),
      dist = Math.sqrt(distSQ),
      force = $div(partA.get$mass() * partB.get$mass(), distSQ),
      ax = $div($mul(force, dx), dist),
      ay = $div($mul(force, dy), dist);
  ...
  function move(partA, i) {
    ...
    for (var partB, j = $add(i, 1); $lt(j, numParticles); j = $add(j, 1)) {
