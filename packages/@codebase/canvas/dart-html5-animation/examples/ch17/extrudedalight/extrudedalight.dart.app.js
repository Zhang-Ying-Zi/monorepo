...
function $bit_and(x, y) {
  return typeof x == "number" && typeof y == "number" ? x & y : x.$bit_and(y);
}
function $bit_or(x, y) {
  return typeof x == "number" && typeof y == "number" ? x | y : x.$bit_or(y);
}
function $div(x, y) {
...
Utils.parseColor = function (color, toNumber) {
  if (null == toNumber ? null == true : toNumber === true) {
    ...
    if (typeof color == "number") {
      ...
      b = $bit_and(color, 255);
      return $add(
        $add($add("#", r.toRadixString(16)), g.toRadixString(16)),
        b.toRadixString(16)
      );
    }
  }
...
extrudedalight.prototype.run = function () {
  ...
  var light = new Light(),
    ...
    triangles = [],
    fl = 250,
    vpX = canvas.get$width() / 2,
    vpY = canvas.get$height() / 2,
    angleX = 0,
    angleY = 0;
...
Point3d.prototype.get$x = function () {
  return this.x;
};
Point3d.prototype.set$x = function (value) {
  return (this.x = value);
};
Point3d.prototype.get$y = function () {
  return this.y;
};
Point3d.prototype.set$y = function (value) {
  return (this.y = value);
};
Point3d.prototype.get$z = function () {
  return this.z;
};
Point3d.prototype.set$z = function (value) {
  return (this.z = value);
};
...
Point3d.prototype.rotateX = function (angleX) {
  var cosX = Math.cos(angleX),
    ...
    y1 = $mul(this.y, cosX) - $mul(this.z, sinX),
    z1 = $mul(this.z, cosX) + $mul(this.y, sinX);
  this.y = y1;
  this.z = z1;
};
Point3d.prototype.rotateY = function (angleY) {
  var cosY = Math.cos(angleY),
    ...
    x1 = $mul(this.x, cosY) - $mul(this.z, sinY),
    z1 = $mul(this.z, cosY) + $mul(this.x, sinY);
  this.x = x1;
  this.z = z1;
};
...
Triangle.prototype.get$lineWidth = function () {
  return this.lineWidth;
};
Triangle.prototype.set$lineWidth = function (value) {
  return (this.lineWidth = value);
};
Triangle.prototype.get$light = function () {
  return this.light;
};
Triangle.prototype.set$light = function (value) {
...
Triangle.prototype.isBackface = function () {
  var cax = $sub(this.pointC.getScreenX(), this.pointA.getScreenX()),
    cay = $sub(this.pointC.getScreenY(), this.pointA.getScreenY()),
    bcx = $sub(this.pointB.getScreenX(), this.pointC.getScreenX()),
    bcy = $sub(this.pointB.getScreenY(), this.pointC.getScreenY());
  return $mul(cax, bcy) > $mul(cay, bcx);
};
Triangle.prototype.getAdjustedColor = function () {
  var _color = Math.parseInt($add("0x", this.color.substring(1))),
    red = _color >> 16,
    green = (_color >> 8) & 255,
    blue = _color & 255,
...
Triangle.prototype.getLightFactor = function () {
  var ab = new Light.withParameters$ctor(
    this.pointA.x - this.pointB.x,
    this.pointA.y - this.pointB.y,
    this.pointA.z - this.pointB.z
  );
  var bc = new Light.withParameters$ctor(
    this.pointB.x - this.pointC.x,
    this.pointB.y - this.pointC.y,
    this.pointB.z - this.pointC.z
  );
  var norm = new Light.withParameters$ctor(
    $mul(ab.get$y(), bc.get$z()) - $mul(ab.get$z(), bc.get$y()),
    -($mul(ab.get$x(), bc.get$z()) - $mul(ab.get$z(), bc.get$x())),
    $mul(ab.get$x(), bc.get$y()) - $mul(ab.get$y(), bc.get$x())
  );
  var dotProd =
    $mul(norm.get$x(), this.light.x) +
    $mul(norm.get$y(), this.light.y) +
    $mul(norm.get$z(), this.light.z);
  var normMag = Math.sqrt(
    $mul(norm.get$x(), norm.get$x()) +
      $mul(norm.get$y(), norm.get$y()) +
      $mul(norm.get$z(), norm.get$z())
  );
  var lightMag = Math.sqrt(
    this.light.x * this.light.x +
      this.light.y * this.light.y +
      this.light.z * this.light.z
  );
  return (
    (Math.acos($div(dotProd, $mul(normMag, lightMag))) / 3.141592653589793) *
...
Light.prototype.get$x = function () {
  return this.x;
};
Light.prototype.set$x = function (value) {
  return (this.x = value);
};
Light.prototype.get$y = function () {
  return this.y;
};
Light.prototype.set$y = function (value) {
  return (this.y = value);
};
Light.prototype.get$z = function () {
  return this.z;
};
Light.prototype.set$z = function (value) {
  return (this.z = value);
};
// ********** Code for top level **************
function main() {
