...
bubbles1.prototype.run = function () {
  ...
  for (var i = 0; $lt(i, numBalls); i = $add(i, 1)) {
  ...
  function draw(ball) {
    var dx = $sub(ball.get$x(), centerBall.get$x()),
      dy = $sub(ball.get$y(), centerBall.get$y()),
      dist = Math.sqrt($mul(dx, dx) + $mul(dy, dy)),
      min_dist = ball.get$radius() + centerBall.get$radius();
    if ($lt(dist, min_dist)) {
      var angle = Math.atan2(dy, dx),
        tx = $add(centerBall.get$x(), $mul(Math.cos(angle), min_dist)),
        ty = $add(centerBall.get$y(), $mul(Math.sin(angle), min_dist));
