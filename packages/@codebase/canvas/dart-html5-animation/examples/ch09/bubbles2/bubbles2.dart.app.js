    ...
    for (
      ...
      if ($lt(dist, min_dist)) {
        var angle = Math.atan2(dy, dx),
          tx = $add(ballA.get$x(), $mul(Math.cos(angle), min_dist)),
          ty = $add(ballA.get$y(), $mul(Math.sin(angle), min_dist)),
          ax = $mul($sub(tx, ballB.get$x()), spring) * 0.5,
          ay = $mul($sub(ty, ballB.get$y()), spring) * 0.5;
