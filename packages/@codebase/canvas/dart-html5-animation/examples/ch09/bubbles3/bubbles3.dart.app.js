...
bubbles3.prototype.run = function () {
  ...
  function checkCollision(ballA, i) {
    for (
      var ballB, dx, dy, dist, min_dist, j = $add(i, 1);
      $lt(j, numBalls);
      j = $add(j, 1)
    ) {
      ballB = balls.$index(j);
      ...
      if ($lt(dist, min_dist)) {
        var tx = $add(ballA.get$x(), $mul($div(dx, dist), min_dist)),
          ty = $add(ballA.get$y(), $mul($div(dy, dist), min_dist)),
          ax = $mul($sub(tx, ballB.get$x()), spring),
          ay = $mul($sub(ty, ballB.get$y()), spring);
