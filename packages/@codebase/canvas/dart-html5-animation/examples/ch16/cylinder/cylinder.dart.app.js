...
StringImplementation.prototype.substring$1 =
  StringImplementation.prototype.substring;
// ********** Code for _Worker **************
...
Utils.colorToRGB = function (color, alpha) {
  ...
  var r = (i >> 16) & 255,
    g = (i >> 8) & 255,
    b = i & 255,
    a = $lt(alpha, 0) ? 0 : $gt(alpha, 1) ? 1 : alpha;
  if (null == a ? null == 1 : a === 1) {
    return $add($add($add($add($add($add("rgb(", r), ","), g), ","), b), ")");
  } else {
    return $add(
      $add(
        $add($add($add($add($add($add("rgba(", r), ","), g), ","), b), ","),
        a
      ),
      ")"
    );
...
cylinder.prototype.run = function () {
  ...
  for (; $lt(i, $sub(numFaces, 1)); i = $add(i, 1)) {
    triangles.add$1(
      new Triangle(
        points.$index(idx),
        points.$index($add(idx, 3)),
        points.$index($add(idx, 1)),
        "#6666cc"
      )
    );
    triangles.add$1(
...
Triangle.prototype.draw = function (context) {
  ...
  context.set$fillStyle(
    (context.set$strokeStyle(($0 = Utils.colorToRGB(this.color, this.alpha))),
    $0)
  );
  context.beginPath();
