/* eslint-disable */
var v2prefix = baseConfig.service.PRIX.substring(
  0,
  baseConfig.service.PRIX.length - 1
);
// var v2prefix = 'http://192.168.1.72/';

/** v2接口调用 */
function asyncV2Service(
  appendurl,
  fdata,
  callback,
  errorcallback,
  ...
  if (
    ($$(".my-card-list") &&
      $$(".my-card-list").css("display") != "none" &&
      fdata &&
      fdata.start > 0) ||
    appendurl == "/homepage/listMenu2"
  ) {
    isSync = true;
  }
  app.request({
    url:
      v2prefix +
      "api/v2/pk/" +
      (window.localStorage.currentPkTag || 0) +
      appendurl,
    method: "POST",
    processData: true,
    ...
    error: function (xhr) {
      ...
      if (xhr.status == 401) {
        ...
        var errtext =
          restext.indexOf("{") == 0 && JSON.parse(restext).error
            ? JSON.parse(restext).error
            : restext;
...
function asyncV2ImgService(appendurl, fdata) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    req.open(
      "POST",
      v2prefix + "api/v2/pk/" + window.localStorage.currentPkTag + appendurl,
      true
    );
    ...
    req.setRequestHeader(
      "Authentication",
      "Bearer " + window.localStorage.xsbtoken
    );
...
function asyncPKService(func, formData, callback, errorcallback) {
  ...
  app.request({
    url:
      ...
      func +
      "?resid=" +
      window.localStorage.currentPkTag,
    method: "POST",
    processData: true,
    contentType: "application/json",
    async: false,
