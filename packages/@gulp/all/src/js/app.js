...
var app = new Framework7({
  ...
  on: {
    routeChange: function (newRoute, previousRoute, router) {
      ...
      $$('.home-menu-item[href="' + currentUrl + '"]').addClass(
        CHILD_MENU_ACTIVE_CLASS
      );
...
if (window.location.search && getUrlQueryString("xsbtoken")) {
  ...
  window.localStorage.pklasturl = getUrlQueryString("redirect")
    ? "#" + getUrlQueryString("redirect")
    : location.hash || "";
  if (getUrlQueryString("areaname")) {
    // 用于外部链接跳转到指定区域热力图
...
function goToLoginSys() {
  var seachurl =
    "?sys=pk" +
    (location.hash ? "&redirect=" + location.hash.replace("#", "") : "");
  window.location.href =
    window.location.protocol +
    "//" +
    window.location.host +
...
function initUserParkingList() {
  ...
  $$(".parking-item").on("click", function () {
    ...
    $$(".main-view-container")
      .removeClass("side-view")
      .removeClass("normal-view");
    $$(".main-view-container").addClass(
      currentPkType === "L" ? "side-view" : "normal-view"
    ); // 增加class，用于场库，路内样式区分
  ...
  if (window.localStorage.xsbtoken) {
    ...
    var findPk = parkingLotList.find(function (m) {
      return m.tag == curPkTag;
    });
    if (curPkTag && findPk) {
      // 有上一次进入的停车场权限，默认再次进入此停车场
...
function initSubMenu(pktag) {
  if (window.localStorage.xsbtoken) {
    ...
    asyncV2Service(
      ...
      function (data) {
        ...
        subMenuListResult.forEach(function (m) {
          ...
          // 设置主菜单选中状态
          var findChildMenu = m.children.find(function (cu) {
            return cu.url === pathurl;
          });
          if (
            m.url == pathurl ||
            findChildMenu ||
            (m.cname == "车辆管理" &&
              pathurl.indexOf("/cars/card/record/") !== -1 &&
              m.children.find(function (cu) {
                return cu.url === "/cars/card";
              })) ||
            (m.cname == "查询" &&
              pathurl.indexOf("/query/charge-warn-detail/") !== -1 &&
              m.children.find(function (cu) {
                return cu.url === "/query/charge-warn-list";
              })) ||
            (m.cname == "商券管理" &&
              pathurl.indexOf("/coupon/detail/") !== -1 &&
              m.children.find(function (cu) {
                return cu.url === "/coupon/coupons";
              }))
          ) {
            hasCurMenu = true;
          ...
          // 热力图打开新的标签页并全屏显示
          var findRLtMenu = m.children.find(function (cu) {
            return cu.url === "/control/park-heat-map";
          });
          if (findRLtMenu) {
        ...
        if (
          !hasCurMenu &&
          pathurl !== "/control/config-heat-map" &&
          pathurl !== "/control/park-heat-map" &&
          pathurl !== "/control/config-led-screen" &&
          pathurl.indexOf("/control/park-map") == -1
        ) {
          var fm = subMenuListResult.find(function (sm) {
            return sm.url === HOME_PAGE_URL;
          });
          if (fm) {
            // 没有该地址权限，加载首页
          ...
          if (isIE()) {
            setTimeout(function () {
              mainView.router.navigate(pathurl, {
                animate: false,
                reloadCurrent: true,
              });
            }, 50);
    ...
    var innerHTML = compiledSubMenuListTemplate({
      menuList: subMenuListResult,
    });
...
if (localStorage.xsbtoken) {
  ...
  if (
    location.href.indexOf("park-heat-map") !== -1 ||
    location.href.indexOf("/control/park-map") !== -1
  ) {
    $$(".home-navbar").hide();
...
$$(".menu-list").on("click touchend", function (e) {
  ...
  // 记录当前点击的菜单地址
  window.localStorage.pklasturl =
    curEle.attr("href") || curEle.attr("data-href") || "";
  // 点击主菜单
  ...
  if (curEle.hasClass("home-menu-item")) {
    ...
    if (
      curEle[0].dataset &&
      curEle[0].dataset.href == "/control/park-heat-map"
    ) {
      openNewWindowByUrl(curEle[0].dataset.href);
    } else if (
      curEle[0].attributes &&
      curEle[0].attributes["data-href"] &&
      curEle[0].attributes["data-href"].value == "/control/park-heat-map"
    ) {
      // 兼容ie10
...
$$(document).on("click touchend", function (e) {
  ...
  if (
    ...
    x.parents(".pks").length === 0 &&
    !x.hasClass("parking-item")
  ) {
    $$("div.sub").hide();
  }
  // 点击日期控件处理
  ...
  if (
    x[0].nodeName !== "BODY" &&
    !x.hasClass("date-item") &&
    x.parents("#ui-datepicker-div").length === 0 &&
    !x.hasClass("ui-datepicker") &&
  ...
  if (x.hasClass("pixel")) {
    ...
    app.ppp = app.popup.create({
      ...
      content:
        '<div class="popup pix-pop" style="position: absolute;margin:0 !important;width: 180px;height: 80px;top: 60px;left: 0;border-radius: 10px;white-space: nowrap;">' +
...
function changeMenuDisplayAndStyle(len) {
  ...
  var parkNameWidth =
    ($$("#topnav1").css("width").split("px")[0] - 0).toFixed(0) - 0 + 2 + 70; // 70：退出宽度
