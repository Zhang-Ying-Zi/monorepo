...
async function releaseComponents({ webDomian, nameSpace, gitUrl, name, baseApi }) {
  ...
  const sh = new Shell();
  const componentConfig = require(`${process.cwd()}/vslz.config.js`);
  const config = {
    ...componentConfig,
    config: [],
  };
  ...
  sh.shell.ls("packages").forEach((file) => {
    if (file.indexOf(".") === -1) {
      ...
      if (!json.name || !json.version || !json.description) {
        console.error(
          `${rootPath}/packages/${file} 存在不合规范的package.json, 必须包含name、version、description属性`
        );
        process.exit(0);
      }
      // 组件发布按照 组件名+组件版本 的形式进行发布，比如 vslz-global-banner.0.0.1.umd.js
