...
const actions = {
  returnConfig({ commit, state }, { targetConfig, pageData, releaseStatus, commonComponents, save = true }) {
    // 保存页面初始值
    ...
    if (currentIndex === -1) {
      ...
      if (component && component.name === "coco-components-loader") {
        currentComponentSchema = state.pageConfig.remoteComponents.filter(
          (c) => `${c.name}.${c.version}` === component.config.name
        )[0];
      }
